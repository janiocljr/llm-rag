...
class MemoryOrchestrator:
    ...
    def save_turn(self, session_id: str, question: str, answer: str, tags: Optional[list[str]] = None) -> dict:
        q_embedding = self._embed_text(question)
        a_embedding = self._embed_text(answer)

        q_chroma_id = self._memory_store.save(text=question, embedding=q_embedding, memory_type="question", session_id=session_id, tags=tags or [])
        a_chroma_id = self._memory_store.save(text=answer, embedding=a_embedding, memory_type="answer", session_id=session_id, tags=tags or [])
        ...
        content = f"## Pergunta\n\n{question}\n\n## Resposta\n\n{answer}"
        mongo_id = self._doc_store.create(title=question[:100], content=content, doc_type="conversation", tags=tags or [], session_id=session_id, chroma_ids=[q_chroma_id, a_chroma_id])
    ...
    def save_task(self, title: str, description: str = "", priority: str = "medium", tags: Optional[list[str]] = None, session_id: Optional[str] = None) -> dict:
        ...
        task_id = self._task_store.create(title=title, description=description, priority=priority, tags=tags or [], session_id=session_id, chroma_id=chroma_id)
        mongo_id = self._doc_store.create(title=f"[TASK] {title}", content=content, doc_type="task", tags=tags or [], session_id=session_id, chroma_ids=[chroma_id], metadata={"task_id": task_id})
        return {"task_id": task_id, "mongo_id": mongo_id, "chroma_id": chroma_id}

    def memory_stats(self) -> dict:
        return {"chroma": {"pdf_embeddings": self._pdf_store.size, "chat_memory": self._memory_store.size}, "mongodb": self._doc_store.stats()}
