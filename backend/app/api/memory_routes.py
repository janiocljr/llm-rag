...
async def save_note(body: SaveNoteRequest, request: Request):
    mem = _mem(request)
    result = mem.save_note(title=body.title, content=body.content, session_id=body.session_id, tags=body.tags)
...
async def save_knowledge(body: SaveKnowledgeRequest, request: Request):
    mem = _mem(request)
    result = mem.save_knowledge(title=body.title, content=body.content, source_file=body.source_file, session_id=body.session_id, tags=body.tags)
    return {"ok": True, **result}
...
async def save_task(body: SaveTaskRequest, request: Request):
    mem = _mem(request)
    result = mem.save_task(title=body.title, description=body.description, priority=body.priority, tags=body.tags, session_id=body.session_id)
    return {"ok": True, **result}


@router.get("/documents", summary="List or search knowledge base documents")
async def list_documents(request: Request, doc_type: Optional[str] = Query(default=None), tags: Optional[str] = Query(default=None, description="Comma-separated tags"), q: Optional[str] = Query(default=None, description="Full-text search query"), limit: int = Query(default=30, ge=1, le=200),):
    mem = _mem(request)
...
async def recall_context(body: RecallRequest, request: Request):
    mem = _mem(request)
    memories = mem.reconstruct_context(question=body.question, session_id=body.session_id, top_k=body.top_k, threshold=body.threshold)
